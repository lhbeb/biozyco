...
import { sendTelegramMessage, formatLinkClickNotification } from '@/lib/telegram';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { username, linkTitle, linkUrl } = body;

    if (!username || !linkTitle || !linkUrl) {
      return NextResponse.json(
        { error: 'Username, linkTitle, and linkUrl are required' },
        { status: 400 }
      );
    }
    ...
    const notificationMessage = formatLinkClickNotification({
      username,
      linkTitle,
      linkUrl,
      userIp: ip,
    ...
    console.error('Error in link click tracking endpoint:', error);
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({ message: 'Link click tracking endpoint' });
}
